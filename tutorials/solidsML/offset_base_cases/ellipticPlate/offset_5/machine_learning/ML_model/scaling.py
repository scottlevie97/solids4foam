...
for model_number in np.arange(1, amount_models+1):
    ...
    scalerI = pickle.load(open('scalerI_' + str(model_number) + '.pkl','rb'))
    scalerO = pickle.load(open('scalerO_' + str(model_number) + '.pkl','rb'))

    mean_tmp = np.hstack((scalerI.mean_, scalerO.mean_))
    std_tmp = np.hstack((scalerI.scale_, scalerO.scale_))
    
    data_tmp = np.vstack((mean_tmp, std_tmp))
    ...
    mean = np.hstack((mean, mean_tmp))
    std = np.hstack((std, std_tmp))



# names = np.array([])
# for i in np.arange(1,len(mean_tmp)/3):
...
# df = pd.DataFrame(data, columns = names, index = ["mean", "std"])
