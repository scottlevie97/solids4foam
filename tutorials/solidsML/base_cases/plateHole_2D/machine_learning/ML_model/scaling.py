...
names = np.array([])
for i in np.arange(1,len(mean)/2):
...
mean_string = str()
for i in np.arange(0,len(mean)/2):

    mean_string =  mean_string + "\t\t(" + str(mean[2*int(i)]) + "\t\t"+ str(mean[2*int(i) + 1]) + "\t\t 0 )\n"

std_string = str()
for i in np.arange(0,len(std)/2):

    std_string =  std_string + "\t\t(" + str(std[2*int(i)]) + "\t\t"+ str(std[2*int(i) + 1]) + "\t\t 0 )\n"
