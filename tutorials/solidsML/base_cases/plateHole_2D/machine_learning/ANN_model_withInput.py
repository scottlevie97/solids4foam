...
def inputsoutputsAllfiles(trainIterations, filepatharray, Timestepsize, maxtimestep):
    ...
    def xyz(iteration, filepath, Timestepsize, Timestep):
        ...
        if Time % 1 == 0:
            ...
            Time = round(Time, 2)
        my_file = open(filepath + str(Time) +
                       "/solidsCellD_iteration"+str(iteration))
    ...
    def maxIteration(Timestep, Timestepsize, filepath):
        ...
        # splits into new dataframe on spaces
        new = df['One'].str.split(' ', expand=True)

        # labels new columns and inserts in dataframe
        df['TimeStep'] = new[0].astype('float')
        # labels new columns and inserts in dataframe
        ...
        # labels new columns and inserts in dataframe
        df["IterationNumber"] = new[4].astype('float')
        ...
        for i in range(len(df)):  # loops through residual file
            if df['TimeStep'][i] == Time:   # Finds values for a timestep
                ...
                # Creates arrays of iteration numbers
                array.append(x)
    ...
    def inputsoutputsAllTimesteps(trainIterations, filepath, Timestepsize):
        ...
        def inputsoutputsTimestep(trainIterations, filepath, Timestepsize, Timestep):
            ...
            # Gives matrix a starting value which is removed later
            matrix = np.array(np.vstack((x, y)))

            # loops for each iteration up to trainIteration
            for i in range(0, trainIterations):
                x, y, z = xyz(i, filepath, Timestepsize, Timestep)
                # gives array with x, y, z values for each iteration
                arr = np.vstack((x, y))
                matrix = np.concatenate((matrix, arr))   # combines arrays

            inputs = matrix[2:, :]  # removes initial value

            xmax, ymax, zmax = xyz(max, filepath, Timestepsize, Timestep)
            # array of values at max iteration
            outputs = np.vstack((xmax, ymax))

            # multiple columns : (x1, y1, z1, x2, y2, z2 ...)
            inputs = np.transpose(inputs)
            outputs = np.transpose(outputs)

            return inputs, outputs

        # first row to be removed after
        arrinputs = np.ones((1, trainIterations*2))
        arroutputs = np.ones((1, 2))

        # combines inputs and outputs of all timesteps

        for Timestep in range(1, maxtimestep+1):
            inputs, outputs = inputsoutputsTimestep(
                trainIterations, filepath, Timestepsize, Timestep)
    ...
    for i in range(0, len(filepatharray), 1):
        inputs, outputs = inputsoutputsAllTimesteps(
            trainIterations, filepatharray[i], Timestepsize)
        arrinputs = np.vstack((arrinputs, inputs))
        arroutputs = np.vstack((arroutputs, outputs))
...
inputs, outputs = inputsoutputsAllfiles(
    trainIterations, filepatharray, Timestepsize, maxtimestep)  # define model
...
hidden_size = ((len(X[0, :])) + 2)/2

# Hidden layer the mean of input and output shape
...
if answer_trainOrLoadModel == ("t"):
    ...
    model = Sequential()
    model.add(Dense(hidden_size, input_dim=len(X[0, :]),
