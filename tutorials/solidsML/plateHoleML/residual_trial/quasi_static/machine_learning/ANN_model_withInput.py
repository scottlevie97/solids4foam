...
def inputsoutputsAllfiles(trainIterations, filepatharray, Timestepsize, maxtimestep, filename):

    def xyz(iteration, filepath, Timestepsize, Timestep, filename):
        ...
        if Time%1 == 0:
            ...
            Time = round(Time,2)
        my_file = open(filepath + str(Time) +"/" + filename + str(iteration))
        ...
        # Hard code work around
        if (iteration == 298) & (filename == "residualD_iteration"):
            x = np.zeros((1000))
            y = np.zeros((1000))
            z = np.zeros((1000))
        else:
    ...
    def inputsoutputsAllTimesteps(trainIterations, filepath, Timestepsize):
        ...
        def inputsoutputsTimestep(trainIterations, filepath, Timestepsize, Timestep):
            ...
            x,y,z = xyz(1, filepath, Timestepsize, Timestep, filename)
            ...
            for i in range(0,trainIterations): # loops for each iteration up to trainIteration
                x,y,z = xyz(i,filepath, Timestepsize, Timestep, filename)
            ...
            xmax, ymax, zmax = xyz(max, filepath, Timestepsize, Timestep, filename)
...
filename_1 = "solidsCellD_iteration"
filename_2 = "residualD_iteration"
...
inputs_1, outputs_1 = inputsoutputsAllfiles(trainIterations, filepatharray, Timestepsize, maxtimestep, filename_1)# define model
inputs_2, outputs_2 = inputsoutputsAllfiles(trainIterations, filepatharray, Timestepsize, maxtimestep, filename_2)# define model

inputs = np.hstack((inputs_1, inputs_2))
outputs = np.hstack((outputs_1, outputs_2))
...
if answer_trainOrLoadModel == ("t"):
    ...
    plt.legend()
    if trainingGraph: plt.savefig('ML_model/training.png')
