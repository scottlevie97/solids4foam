...
def cellCentres(filepath, Timestepsize, Timestep):
    ...
    if Time%1 == 0:
        ...
        Time = round(Time,2)
    my_file = open(filepath + str(Time) +"/cellCentres")
    string_list = my_file.readlines()
    index=18
    string_list = string_list[index+2:len(string_list)] 
    string_list = string_list[0:len(string_list)-4] 
...
#  Needs to be changed for transient problems
inputs = np.hstack([inputs, cellCentres(filepath1, Timestepsize, 1)])
...
if answer_trainOrLoadModel == ("t"):
    ...
    model = Sequential()
    model.add(Dense(20, input_dim=len(X[0,:]), 
...
# for i in np.arange(0, len(scaledPrediction[:, 0])):

#     print(inputs[i, :])
#     print(scaledX[i,:])
#     print(scaledPrediction[i,:])
#     print(prediction[i,:])  
#     print(" ")
